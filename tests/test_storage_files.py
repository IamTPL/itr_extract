...
from config.constants import INPUT_FILENAME, ECONSENT_FILENAME, VOUCHER_FILENAME
...
def test_voucher_uses_constant_filename(tmp_path, monkeypatch):
    monkeypatch.setattr(fs, "_files_root", lambda: tmp_path)
    u, j = uuid4(), uuid4()
    fs.write_voucher(u, j, b"y")
    assert (fs.job_dir(u, j) / VOUCHER_FILENAME).exists()
